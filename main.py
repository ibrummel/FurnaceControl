...
import nidaqmx
import numpy as np
from src.ui.main_ui import Ui_Dialog
import sys
...
class FurnaceLogger(QDialog):
    ...
    def init_fields(self):
        self.ui.combo_monitor_tc.addItems(list(self.tc_types.keys()))
        ...
        self.ui.combo_profile_number.addItems([str(x) for x in np.linspace(1, 99, 99)])
        self.ui.combo_profile_tracking.addItems(list(ENUM.write.ramp_soak_tracking.keys()))
        self.update_fields()

    def update_fields(self):
        if not self.ui.combo_monitor_tc.hasFocus():
            self.ui.combo_monitor_tc.setCurrentText(self.external_tc.ai_thrmcpl_type.name)

        if not self.ui.combo_controller_tc.hasFocus():
        ...
        if not self.ui.combo_output_mode.hasFocus():
            self.ui.combo_output_mode.setCurrentText(self.controller.get_output_mode(3))

        if not self.ui.combo_profile_number.hasFocus():
            self.ui.combo_profile_number.setCurrentText(str(self.controller.get_current_profile_number()))

        if not self.ui.combo_profile_tracking.hasFocus():
            self.ui.combo_profile_tracking.setCurrentText(self.controller.get_ramp_soak_tracking_mode())

    def init_connections(self):
        self.ui.combo_monitor_tc.currentTextChanged.connect(self.set_external_tc_type)
