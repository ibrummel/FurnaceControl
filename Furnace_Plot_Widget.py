...
class FurnacePlotWidget(QFrame):
    def __init__(self, lines_info: dict, parent=None,):
        super(FurnacePlotWidget, self).__init__(parent)
    ...
    def remove_line(self, line_key: str, draw_frame=False):
        self.canvas.remove_line(line_key, draw_frame)

    def clear_data(self):
        for line_key in self.canvas.lines_info.keys():
            self.canvas.lines_info[line_key]['x'] = []
            self.canvas.lines_info[line_key]['y'] = []


class FurnacePlotCanvas(FigCanvas):
    def __init__(self, parent, lines_info={}, nrows=1, ncols=1, line_width=2,):
        self.parent = parent
    ...
    def init_axes(self):
        for i, subplot_row in enumerate(self.axes):
            for j, subplot in enumerate(subplot_row):
                ...
                self.change_axes_labels([i, j], ['Time', 'Temperature'])

    def _check_axes_assignment(self, check_key=None):
        for line_key in self.lines_info.keys():
            if check_key is not None and line_key != check_key:
                continue
            else:
                ...
                try:
                    self.lines_info[line_key]['subplot_row'] = int(self.lines_info[line_key]['subplot_row'])
                    if self.lines_info[line_key]['subplot_row'] > self.axes.shape[0] - 1:
                        print("The line labeled {} had subplot row index outside the current number of subplot rows. "
                              "This line has been moved to the plot at [0,0]".format(line_key))
                        self.lines_info[line_key]['subplot_row'] = 0
                        self.lines_info[line_key]['subplot_col'] = 0
                ...
                try:
                    self.lines_info[line_key]['subplot_col'] = int(self.lines_info[line_key]['subplot_col'])
                    if self.lines_info[line_key]['subplot_col'] > self.axes.shape[1] - 1:
                        print("The line labeled {} had subplot column index outside the current number of subplot "
                              "columns. This line has been moved to the plot at [0,0]".format(line_key))
                        self.lines_info[line_key]['subplot_row'] = 0
                        self.lines_info[line_key]['subplot_col'] = 0
                except ValueError as err:
    ...
    def add_data(self, line_key: str, point: list, draw_frame=False):
        try:
            self.lines_info[line_key]['x'].append(point[0])
            self.lines_info[line_key]['y'].append(point[1])
        except KeyError as err:
    ...
    def add_line(self, line_key: str, line_info: dict, draw_frame=False):
        ...
        self._check_axes_assignment(check_key=line_key)
        if draw_frame:
            self._draw_frame()

    def remove_line(self, line_key: str, draw_frame=False):
        try:
            self.lines_info[line_key]
        except KeyError as err:
